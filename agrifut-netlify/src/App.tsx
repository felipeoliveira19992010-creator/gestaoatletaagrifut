...
const docAttachmentStyles = `@page{size:A4;margin:6mm}.doc-page{display:flex;flex-direction:column;gap:8px;background:white}.doc-page h2{font-size:16px;color:#1B2A4A;margin:0;border-bottom:3px solid #F5C518;padding-bottom:6px}.doc-note{font-size:10px;color:#64748B;margin:0 0 4px}.doc-img-grid{flex:1;display:grid;gap:8px;min-height:0}.doc-count-1{grid-template-rows:minmax(0,1fr)}.doc-count-2{grid-template-rows:repeat(2,minmax(0,1fr))}.doc-count-3{grid-template-rows:repeat(3,minmax(0,1fr))}.doc-count-many{grid-template-columns:repeat(2,minmax(0,1fr));grid-auto-rows:minmax(0,1fr)}.doc-file{border:1px solid #CBD5E1;border-radius:8px;padding:7px;display:flex;flex-direction:column;min-height:0;overflow:hidden;break-inside:avoid;page-break-inside:avoid}.doc-caption{display:flex;justify-content:space-between;gap:10px;align-items:flex-start;margin-bottom:5px}.doc-caption strong{font-size:11px;color:#1B2A4A;text-transform:uppercase}.doc-caption span{font-size:9px;color:#64748B;text-align:right;word-break:break-all}.doc-img-wrap{flex:1;min-height:0;display:flex;align-items:center;justify-content:center;overflow:hidden;background:#F8FAFC;border-radius:6px}.doc-img-wrap img{max-width:100%;max-height:100%;width:auto;height:auto;object-fit:contain;display:block}.doc-pdf-box{flex:1;display:flex;align-items:center;justify-content:center;text-align:center;background:#F8FAFC;border-radius:6px;color:#64748B;font-size:12px;padding:12px}.laudo-page .doc-img-grid{grid-template-rows:1fr}@media screen{.doc-page{min-height:980px;margin-top:28px;border-top:1px solid #E2E8F0;padding-top:16px}.doc-img-grid{height:850px}.laudo-doc .doc-img-wrap{height:auto;min-height:850px}}@media print{.doc-page{height:282mm;max-height:282mm;padding:0;overflow:hidden;break-inside:avoid;page-break-inside:avoid}.doc-page+.doc-page{break-before:page;page-break-before:always}.doc-img-grid{height:259mm;max-height:259mm;overflow:hidden;gap:4mm}.doc-file{padding:3mm}.doc-caption{margin-bottom:1.5mm}.laudo-page{height:282mm;min-height:282mm}.laudo-doc .doc-img-wrap{height:auto;min-height:0}.laudo-doc .doc-img-wrap img{max-height:252mm}}`;
...
const athletePrintLayoutStyles = `@media print{html,body{margin:0!important;padding:0!important;max-width:none!important}.profile-page{display:block;overflow:visible;padding:3mm 2mm 0!important}.profile-page.has-documents{break-after:page;page-break-after:always}.profile-page .hdr{margin-bottom:1.5mm;padding-bottom:1.2mm;gap:3mm}.profile-page .logo{width:13mm;height:13mm}.profile-page .sec{margin-bottom:1.45mm}.profile-page .sec-t{font-size:8px;letter-spacing:.6px;padding-bottom:.4mm;margin-bottom:.9mm}.profile-page .grid{gap:.75mm 4mm}.profile-page .fl{font-size:7.5px;margin:0}.profile-page .fv{font-size:9.2px;line-height:1.12;margin-top:.2mm}.profile-page .termo{font-size:8px;line-height:1.2;padding:1.4mm;margin-bottom:.8mm;border-radius:3px}.profile-page .termo+p{font-size:8px!important;margin:0!important}.profile-page .docs-summary{display:none!important}.profile-page .sig-a{margin-top:1.6mm!important;padding-top:1mm!important;gap:7mm;break-inside:avoid;page-break-inside:avoid}.profile-page .sig-b{font-size:8.5px;padding-top:1mm}.profile-page .sig-b img{max-height:12mm!important;margin-bottom:.8mm!important}.profile-page .sig-space{height:12mm!important}.profile-page .foot{margin-top:1.5mm;padding-top:1mm;font-size:7.5px}}`;
...
<div class="sec docs-summary"><div class="sec-t">Documentos</div><div class="doc-g">
...
export default function App() {
  ...
  const syncCampFinance=async(camp,evOrEvents,ids=null,silent=false)=>{
    if(user?.role!=="admin"){if(!silent)t2("⚠️ Apenas admin pode gerar pendências da competição.");return false;}
      ...
      {selCamp&&(()=>{const c=selCamp;const elig=eligibleForCamp(c).sort(sortName);return(
        <Modal title={"🏆 "+c.nome} onClose={()=>setSelCamp(null)} wide>
          <div style={{display:"grid",gridTemplateColumns:user?.role==="admin"?"1fr 1fr":"1fr",gap:16}}>
            ...
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><p style={{fontWeight:800,fontSize:13,color:N,margin:0,textTransform:"uppercase",letterSpacing:1}}>⚽ Partidas</p>{user?.role==="admin"&&<Btn small color={G} onClick={()=>setShowEv(true)}>+ Evento</Btn>}</div>
              {(c.eventos||[]).length===0?<p style={{color:"#888",fontSize:13}}>Nenhum evento.</p>
                :<div style={{display:"flex",flexDirection:"column",gap:10,maxHeight:340,overflowY:"auto"}}>{c.eventos.map(ev=>(
                  <div key={ev.id} style={{background:"#F8FAFC",borderRadius:10,padding:12,border:"1px solid #E2E8F0"}}>
                    ...
                    <details><summary style={{fontSize:12,cursor:"pointer",color:"#64748B",fontWeight:600}}>Selecionar atletas convocados</summary>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:16}}>
              ...
              <div style={{gridColumn:'1/-1'}}>
                ...
                <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:10}}>
                  {[...new Set([...TAMANHOS,...itemF.tamanhos])].map(t=>{
                    const sel=itemF.tamanhos.includes(t);
                    return <button key={t} onClick={()=>toggleItemSize(t)} style={{background:sel?N:'white',color:sel?G:N,border:`1.5px solid ${N}`,borderRadius:7,padding:'5px 12px',cursor:'pointer',fontWeight:700,fontSize:12}}>{t}</button>;
                  })}
                </div>
                <div style={{display:'flex',gap:6,marginBottom:10}}>
                  <input value={itemSizeInput} onChange={e=>setItemSizeInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")addItemSize();}} placeholder="Adicionar tamanho" style={{border:'1.5px solid #ddd',borderRadius:7,padding:'7px 10px',fontSize:13,outline:'none',flex:1,minWidth:160}}/>
                  <button onClick={addItemSize} style={{background:N,color:G,border:'none',borderRadius:7,padding:'7px 12px',cursor:'pointer',fontWeight:800,fontSize:12}}>+ Adicionar</button>
                </div>
  ...
  return (
    <div className="agrifut-app-shell" style={{fontFamily:"'Roboto',system-ui,sans-serif",minHeight:"100vh",background:"#F1F5F9"}}>
      ...
      {tab==="financeiro"&&(user.role==="admin"||user.financeiro)&&renderFin()}
